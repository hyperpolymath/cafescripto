...
// Replace rsr-template-repo, cafescripto, {{DEPS}} with actuals
...
| `$PREFIX/share/cafescripto/`
| Data files, assets

| `$PREFIX/share/doc/cafescripto/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/cafescripto/config.toml`

Fallback: `$HOME/.config/cafescripto/config.toml`
...
Or via OPSM: `opsm update cafescripto`
...
[source,bash]
----
just install --prefix=/opt/cafescripto-instance1 --config=/etc/cafescripto/instance1.toml
just install --prefix=/opt/cafescripto-instance2 --config=/etc/cafescripto/instance2.toml
----
